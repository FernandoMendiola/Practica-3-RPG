...
int Combat::calcularExperienciaGanadaDelEnemigo() {
    // Aquí implementa la lógica para calcular la experiencia ganada del enemigo
    return 50; // Por ejemplo, se devuelve 50 como un valor ficticio
}
...
void Combat::doCombat() {
    ...
    if (enemies.empty()) {
        ...
        // Llama a la función para calcular la experiencia ganada
        int experienciaGanadaDelEnemigo = calcularExperienciaGanadaDelEnemigo();

        // Llamar al método gainExperience del jugador y pasarle la cantidad de experiencia ganada
        for (auto& jugador : partyMembers) {
            jugador->gainExperience(experienciaGanadaDelEnemigo);
